...
try {
    ...
    $st=$pdo->prepare("INSERT INTO radius_servers (name,host,port,secret) VALUES ('RADIUS laboratório','10.203.30.3',1812,?)"); $st->execute([$radiusSecret]);
    ...
    $st=$pdo->prepare("INSERT INTO partners (code,name,require_auth,portal_mode,access_purpose,nas_id,nas_interface_id,radius_ip,vlan_id,gateway_ip,pool_start,pool_end,dns_servers,dns_name) VALUES ('FIRESPOT-LAB','FireSpot — Laboratório',0,'v3','hybrid',?,?,'10.203.30.3',10,'10.203.10.1','10.203.10.100','10.203.10.199','10.203.10.1','lab-a.hotspot.internal')");
    ...
    foreach ([10=>'A',20=>'B'] as $vlan=>$letter) {
        $st->execute([$partnerId,'LAB-'.$letter,'Cliente '.$letter.' — VLAN '.$vlan,$nasId,$interfaceId,$vlan,"10.203.{$vlan}.1","10.203.{$vlan}.100","10.203.{$vlan}.199","10.203.{$vlan}.1",'lab-'.strtolower($letter).'.hotspot.internal','10.203.30.3',$vlan===10?1:0]);
